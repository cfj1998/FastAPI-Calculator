...
def log(a: float):
    try:
        ...
        raise HTTPException(status_code=400, detail="Trying to take logarithm of a negative number.")
...
def complex_multiplication(z1_real: float, z1_imag: float, z2_real: float, z2_imag: float):
    z1 = complex(z1_real, z1_imag)
    z2 = complex(z2_real, z2_imag)
    return str(z1 * z2)
...
@app.get("/complex_power", tags=["Basic Complex Functions"])
def complex_to_the_power_int(z_real: float, z_imag: float, a_real: float, a_imag: float):
    z = complex(z_real, z_imag)
...
def delete_from_from_complex_memory(id: int, db:Session = Depends(get_db)):
    ...
    if complex_model is None:
        raise HTTPException(status_code=404, detail=f"Complex Number with id {id} not found.")
